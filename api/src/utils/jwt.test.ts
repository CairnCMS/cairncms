...
import type { CairnTokenPayload } from '../../src/types/index.js';
import { verifyAccessJWT, verifyJWT } from '../../src/utils/jwt.js';

const payload: CairnTokenPayload = { role: null, app_access: false, admin_access: false };
const secret = 'test-secret';
const options = { issuer: 'cairncms' };
...
test('Returns the payload of a correctly signed token', () => {
	...
	expect(result['iss']).toBe('cairncms');
...
const RequiredEntries: Array<keyof CairnTokenPayload> = ['role', 'app_access', 'admin_access'];
