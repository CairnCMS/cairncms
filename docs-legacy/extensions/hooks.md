---
description: A guide on how to build custom hooks in CairnCMS.
readTime: 7 min read
---
...
Below is an example of canceling a `create` event by throwing a standard CairnCMS exception.
...
Init hooks execute at a defined point within the life cycle of CairnCMS. Use init hook objects to inject logic into
internal services.
...
Schedule hooks execute at certain points in time rather than when CairnCMS performs a specific action. This is supported
...
- `emitter` — [Event emitter](https://github.com/CairnCMS/cairncms/blob/main/api/src/emitter.ts) instance that can be
