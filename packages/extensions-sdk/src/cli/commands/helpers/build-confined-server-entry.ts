...
export type ConfinedGuestGlobal = 'CairnOperation' | 'CairnEndpoint' | 'CairnHook' | 'CairnBundle';

// The virtual source name esbuild records for the generated bundle entry module.
// The leading NUL makes it unforgeable: no real file path can contain a NUL byte,
// and esbuild records imported files by their resolved on-disk path, so this is the
// only metafile input that can ever match. It is filtered from the containment
// check because it is generated by the build, not a file on disk; its imports, the
// real entry sources, are resolved and checked normally.
const BUNDLE_STDIN_SOURCEFILE = '\0cairn-confined-bundle-entry';

// A server entry source: exactly one of a source file (`input`) or a generated
// in-memory bundle entry module (`stdin`).
type ConfinedServerEntrySource = { input: string; stdin?: never } | { stdin: string; input?: never };

export type BuildConfinedServerEntryOptions = ConfinedServerEntrySource & {
...
// The real bundled inputs to containment-check: every metafile input except the
// generated bundle entry module, which has no on-disk path.
function containedInputs(metafile: { inputs?: Record<string, unknown> } | undefined): string[] {
	return Object.keys(metafile?.inputs ?? {}).filter((key) => key !== BUNDLE_STDIN_SOURCEFILE);
}

// Exactly one of a file entry or a generated entry must be given. A build helper
// this security-sensitive fails closed rather than silently preferring one.
function assertSingleSource(source: { input?: string; stdin?: string }): void {
	if ((source.input === undefined) === (source.stdin === undefined)) {
		throw new ConfinedBuildError('the confined build needs exactly one of an entry file or a generated entry');
	}
}
...
export async function buildConfinedServerEntry(options: BuildConfinedServerEntryOptions): Promise<{ code: string }> {
	...
	try {
		result = await build(confinedEsbuildOptions(rootAbs, options, options.globalName));
	} catch (error) {
		throw new ConfinedBuildError(sanitizeBuildError(error));
	}

	await assertContained(containedInputs(result.metafile), rootAbs, realRoot);
...
export type WatchConfinedServerEntryOptions = ConfinedServerEntrySource & {
	root: string;
...
export async function watchConfinedServerEntry(
	...
	const watcher = await context({
		...confinedEsbuildOptions(rootAbs, options, options.globalName),
		plugins: [
			{
...
function confinedEsbuildOptions(
	rootAbs: string,
	source: { input?: string; stdin?: string },
	...
	if (source.stdin !== undefined) {
		// Imports in the generated entry resolve from the package root, the same place
		// a file entry would.
		return {
			...common,
			stdin: { contents: source.stdin, resolveDir: rootAbs, sourcefile: BUNDLE_STDIN_SOURCEFILE, loader: 'js' },
		};
	}

	return { ...common, entryPoints: [resolve(rootAbs, source.input!)] };
}
