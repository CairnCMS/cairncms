<template>
	<div
		...
		:class="{ disabled, transition: !disabled && !dragging && !options?.disableTransition }"
	>
		<slot />

		<div
			v-if="!disabled && targetIsVisible"
			class="grab-bar"
...
watch(
	[internalWidth, target, () => props.maxWidth, () => props.disabled],
	([width, target, maxWidth, disabled]) => {
		if (!target) return;
