...
import { Filter, Item } from '@cairncms/types';
import { getEndpoint, toArray } from '@cairncms/utils';
...
export function useRelationMultiple(
	...
	function useSelected() {
		...
		async function loadSelectedDisplayO2M(relation: RelationO2M) {
			...
			fetchedSelectItems.value = await fetchAll<Record<string, any>>(getEndpoint(targetCollection), {
				params: {
					fields: Array.from(fields),
					filter: {
						[targetPKField]: {
