/** @TODO replace with import from @cairncms/utils */
import { getFilterOperatorsForType } from '../../utils/get-filter-operators-for-type';
import type { ClientFilterOperator } from '@cairncms/types';
...
// Overcome MySQL / Maria between behaving like "> x <" instead of ">= x =<"
const floatInconsistencyRange = 0.0001;
				...
				return [
					{
						...
						filter: {
							[`_${filter}`]: [
								sortedPossibleValues[0] - floatInconsistencyRange,
								sortedPossibleValues[middleIndex] + floatInconsistencyRange,
							],
						},
						validatorFunction: getValidatorFunction(filter),
						emptyAllowedFunction: getEmptyAllowedFunction(filter),
...
const empty_nbetween = (inputValue: any, possibleValues: any): boolean => {
	if (Array.isArray(possibleValues)) {
		for (const value of possibleValues) {
			if (value < Number(inputValue[0]) || value > Number(inputValue[1])) {
				return false;
			}
		}
